...
Transforme o seguinte trecho da transcrição em texto formal, claro, detalhado e narrativo, em norma culta, mantendo todas as informações e decisões.

IMPORTANTE:
- Não coloque cabeçalhos, títulos, datas, nomes de presidente ou secretário.
- Apenas transforme o conteúdo do trecho em texto formal, narrativo e coeso.
...
def salvar_texto_docx(texto, caminho_arquivo):
    doc = docx.Document()
...
def main():
    ...
    # Junta os textos gerados separados por 2 quebras de linha
    texto_final = "\n\n".join(textos_gerados)

    salvar_texto_docx(texto_final, "ata_gerada_parcial.docx")
